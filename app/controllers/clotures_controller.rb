...
class CloturesController < ApplicationController
  ...
  def index
    ...
    total_ventes_especes = Vente.where(date_vente: Date.today.all_day, annulee: [false, nil]).sum(:espece)
  ...
  def show
    ...
    ventes = Vente.includes(ventes_produits: :produit).where(date_vente: @cloture.date.all_day, annulee: [false, nil])
  ...
  def cloture_mensuelle
    ...
    clotures_jour = Cloture.where(categorie: "journalier", date: date.beginning_of_month..date.end_of_month)
    if clotures_jour.empty?
      redirect_to clotures_path, alert: "❌ Aucune clôture journalière trouvée pour ce mois."
      return
    end

    ventes_annulees_mois = Vente.where(date_vente: date.beginning_of_month..date.end_of_month, annulee: true)
  ...
  def imprimer
    cloture = Cloture.find(params[:id])
    jour = cloture.date
    ventes_annulees = Vente.where(date_vente: jour.all_day, annulee: true)
    ...
    total_articles = if cloture.categorie == "mensuelle"
      Vente.includes(ventes_produits: :produit)
    ...
    ventes = Vente.includes(:client, ventes_produits: :produit).where(date_vente: jour.all_day, annulee: [false, nil])
  ...
  def preview
    ...
    # ✅ Nouveau calcul des totaux par mode de paiement
    total_cb      = ventes.sum(:cb).to_d
    total_amex    = ventes.sum(:amex).to_d
    total_especes = ventes.sum(:espece).to_d
    total_cheque  = ventes.sum(:cheque).to_d
